...
\frametitle{Adaptive Scheduling}
\bigskip
Given an initial schedule $\tblue{(S, T)}$ and a triple of bad luck $\tblue{(I_f, I_{\delta}, I_{f|\delta})}$, can get a final realization $\tblue{(S', T')}$ by solving a series of optimization problems.
\end{frame}
